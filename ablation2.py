...
class Fixed_Graph_Conv(nn.Module):
    """Adaptive_Graph_Conv with only the adaptive propagation branch removed."""
    ...
    def forward(self, x, adj):
        ...
        out_p = torch.einsum('ij, bjc -> bic', adj, x)
        out_p = self.W_p(out_p)
        return res + self.drop(torch.nn.functional.silu(out_p))
...
class Mamba_FGCN(ST_Mamba_Model):
    """Full Mamba model retaining only the fixed graph propagation branch."""
    def __init__(self, adj=None):
        super().__init__(use_gnn=True, temporal_type='mamba', fusion_mode='advanced', adj=adj)
        adaptive_layer = self.spatial_net
        fixed_layer = Fixed_Graph_Conv(self.h_dim, dropout=config.dropout)
        fixed_layer.W_p.load_state_dict(adaptive_layer.W_p.state_dict())
        fixed_layer.W_r.load_state_dict(adaptive_layer.W_r.state_dict())
        fixed_layer.norm.load_state_dict(adaptive_layer.norm.state_dict())
        self.spatial_net = fixed_layer
...
TIME_COMPLEXITY_MAP = {
    ...
    'Mamba-FGCN': 'O(B·(T·N)·d + T·|E_fixed|) [固定八邻域图与完整时空门控]',
...
def main():
    ...
    experiments = {
        ...
        'Mamba-FGCN': {'gnn': True,  'temporal': 'mamba', 'fusion': 'advanced', 'fixed_graph_only': True}
    }
    ...
    for name, cfg in experiments.items():
        ...
        for i in range(num_runs):
            ...
            if cfg.get('fixed_graph_only', False):
                model = Mamba_FGCN(adj=adj).to(config.device)
            else:
    ...
    experiment_groups = {
        "Ablation_Study": {
            "title": "Mamba-AGCN 消融实验",
            "models": ['Mamba-Only', 'Mamba-FGCN', 'Mamba-U-GNN', 'Mamba-GNN']
        }
    }
